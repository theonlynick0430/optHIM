...
def bfgs_update(x, grad_x, hess_x_prev, eps_sy, x_prev=None, grad_x_prev=None):
    """
    Performs a BFGS update of the Hessian approximation.
    ...
    if x_prev is not None and grad_x_prev is not None:
        ...
        if curv_cond > eps_sy * torch.norm(s) * torch.norm(y):
            # compute BFGS update
            term1 = - hess_x_prev @ torch.outer(s, s) @ hess_x_prev / (s @ hess_x_prev @ s)
            term2 = torch.outer(y, y) / curv_cond
            hess_x = hess_x_prev + term1 + term2
    return hess_x

def bfgs_update_inv(x, grad_x, inv_hess_x_prev, eps_sy, x_prev=None, grad_x_prev=None):
    """
    Performs a BFGS update of the inverse Hessian approximation.
    ...
    if x_prev is not None and grad_x_prev is not None:
        ...
        if curv_cond > eps_sy * torch.norm(s) * torch.norm(y):
            ...
            rho = 1.0 / curv_cond
            term1 = (I - rho * torch.outer(s, y)) @ inv_hess_x_prev @ (I - rho * torch.outer(y, s))
            term2 = rho * torch.outer(s, s)
            inv_hess_x = term1 + term2
    return inv_hess_x
